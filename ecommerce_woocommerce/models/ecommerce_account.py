...
class EcommerceAccount(models.Model):
    ...
    def _fetch_order_from_ecommerce_by_order_ref(self, ecommerce_order_ref):
        ...
        if not order:
            raise ECommerceApiError("This order is not exist from woocommerce side")
        structured_order = self._wc_build_order_structure(order)
            ...
            'shipping_lines': [
                {
                    ...
                    'price_unit': shipping_line.get('total'),
    ...
    def _wc_prepare_order_line_data(self, order_lines):
        ...
        for line in order_lines:
            subtotal = float(line.get('subtotal', 0.0))  # 100
            total = float(line.get('total', 0.0))  # 90
            quantity = line.get('quantity', 0)  # 1
            subtotal_tax = float(line.get('subtotal_tax', 0.0))  # 18
            discount_amount = subtotal - total
            tax_percentage = (100 * subtotal_tax) / subtotal if subtotal else 0
            discount_tax = (discount_amount * tax_percentage) / 100
            line_data = {
                'id': line.get('id'),
                ...
                'price_unit': subtotal / quantity if quantity else subtotal,
