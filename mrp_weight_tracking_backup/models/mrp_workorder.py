...
class MrpWorkorder(models.Model):
    ...
    def action_capture_start_weight(self):
        ...
        return {
            ...
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref('mrp_weight_tracking.view_workorder_start_weight_form').id,
            'target': 'new',
        }
    ...
    def action_capture_end_weight(self):
        ...
        return {
            ...
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref('mrp_weight_tracking.view_workorder_end_weight_form').id,
            'target': 'new',
        }
    ...
    def _create_weight_delta_stock_move(self):
        ...
        # Create minimal stock move for paint consumption
        move_vals = {
            'name': _('Paint Consumption - %s') % production.name,
        ...
        # Directly update stock quantity instead of using complex stock moves
        # This ensures the paint inventory is reduced immediately
        ...
        # Now process the stock move for record keeping
        move._action_confirm()
        
        # Create move line with the consumed quantity
    ...
    def do_finish(self):
        """Override to ensure weight tracking is completed"""
        if self.has_weight_tracking:
            if not self.weight_captured_at_start:
                raise UserError(_('Please capture starting weight before finishing.'))
            if not self.weight_captured_at_end:
                raise UserError(_('Please capture ending weight before finishing.'))
