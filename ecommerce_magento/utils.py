...
def make_paginated_request(ec_account: object, method, route, params, page_size=100, current_page=1):
    """Handle pagination when fetching the Magento objects."""
    items = []
    total_count = None
...
def make_request(ec_account, method, route, params=None, payload=None, **kwargs):
    """Send a request to the Magento server using the REST API.
    ...
    if ec_account.magento_auth_method == "token":
        headers["Authorization"] = f"Bearer {ec_account.magento_access_token}"
        ...
        headers["Authorization"] = build_oauth_header(
            ...
            token=ec_account.magento_oauth_token,
            token_secret=ec_account.magento_oauth_token_secret,
            query_params=params,
        )
    try:
        ...
        if ec_account.magento_auth_method == "token" and e.response.status_code == 401 and kwargs.get("refresh_token_on_401", True):
            ...
            access_token = get_admin_access_token(base_url, ec_account.magento_username, ec_account.magento_password)
            ec_account.magento_access_token = access_token
