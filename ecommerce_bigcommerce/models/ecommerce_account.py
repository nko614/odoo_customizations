...
class ecommerceAccount(models.Model):
    ...
    def _fetch_products_from_ecommerce(self):
        ...
        if not (response and "data" in response):
            raise ECommerceApiError(self.env._("Unexpected Error, something went wrong."))
    ...
    def _fetch_order_from_ecommerce_by_order_ref(self, ecommerce_order_ref):
        ...
        return self._prepare_order_structure(order)
    ...
    def _prepare_order_structure(self, order):
        ...
        order_data = {
            ...
            'financial_status': self.get_actual_payment_status(order),
        ...
        for consignment in order.get('consignments', []):
            for shipping in consignment.get('shipping', []):
                for order_line in shipping.get('line_items', []):
                    ...
                    price_unit = order_line.get('price_inc_tax') if self.tax_included else order_line.get('price_ex_tax')
                    price_subtotal = order_line.get('total_inc_tax') if self.tax_included else order_line.get('total_ex_tax')
        ...
        for consignment in order.get('consignments', []):
            for shipping in consignment.get('shipping', []):
                if shipping.get('base_cost', 0):
                    price_unit_ex_tax = float(shipping.get('cost_ex_tax', 0.0)) + float(shipping.get('handling_cost_ex_tax', 0.0))
                    price_unit_inc_tax = float(shipping.get('cost_inc_tax', 0.0)) + float(shipping.get('handling_cost_inc_tax', 0.0))
                    tax_amount = float(shipping.get('cost_tax', 0.0)) + float(shipping.get('handling_cost_tax', 0.0))
                    final_price_unit = price_unit_inc_tax if self.tax_included else price_unit_ex_tax
    ...
    def _update_picking_to_bigcommerce(self, pickings):
        ...
        if not bigcommerce_order_id or not tracking_number:
            raise ECommerceApiError(self.env._("Missing BigCommerce order ID or tracking number."))
        ...
        if not order_response or 'consignments' not in order_response:
            raise ECommerceApiError(self.env._("Unable to fetch order consignments for shipment creation."))
        ...
        if not order_address_id:
            raise ECommerceApiError(self.env._("No shipping address found for creating shipment."))
    ...
    def get_actual_payment_status(self, order):
        """
        Compute the actual payment status of a BigCommerce order.

        Returns one of:
        paid, pending, authorized, partially_paid,
        ...
        total = float(order.get("total_inc_tax", 0.0))
        refunded = float(order.get("refunded_amount", 0.0))

        if payment_status in {"void", "voided", "void pending"}:
            return "VOIDED"

        if payment_status in {"expired"}:
            return "EXPIRED"
        ...
        if payment_status in {"authorized", "held for review"}:
            return "AUTHORIZED"
        ...
        if payment_status in {"pending", "capture pending"}:
            return "PENDING"

        if order_status in {"awaiting payment", "incomplete"}:
            return "PENDING"
