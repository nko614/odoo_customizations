...
from odoo.addons.ecommerce_prestashop import const as prestashop_consts
...
class ECommerceAccount(models.Model):
    ...
    def action_connect(self):
        ...
        def match_store():
            ...
            if not matching_shop:
                raise ValidationError(_("Invalid Store Name."))
        ...
        if self.channel_code == 'prestashop':
            try:
                ...
                raise UserError(_("Failed to connect ECommerce account: ")) from error
    ...
    def _get_product_url(self, offer):
        ...
        return f'{base_url}/sell/catalog/products/{product_id}'
    ...
    def _fetch_order_from_ecommerce_by_order_ref(self, ecommerce_order_ref):
        ...
        if isinstance(response_order, list) and len(response_order) == 0:
            err_msg = "Order not found in Prestashop."
            raise ECommerceApiError(err_msg)
        return self._process_order(response_order[0])
    ...
    def _process_order(self, so):
        ...
        def get_order_address(customer_id, address_id):
            ...
            def get_customer_info(customer_id):
                ...
                return next((customer for customer in customers if str(customer.get('id')) == str(customer_id)), {})
        ...
        def get_shipping_lines(so):
            return [{
                ...
                'price_unit': float(so.get('total_shipping_tax_incl', 0.0)) if tax_included else float(so.get('total_shipping_tax_excl', 0.0)),
                'tax_amount': float(so.get('total_shipping_tax_incl', 0.0)) - float(so.get('total_shipping_tax_excl', 0.0)),
            }]

        def get_discount_lines(so):
            return [{
                'price_unit': float(so.get('total_discounts_tax_incl', 0.0)) if tax_included else float(so.get('total_discounts_tax_excl', 0.0)),
                'tax_amount': float(so.get('total_discounts_tax_incl', 0.0)) - float(so.get('total_discounts_tax_excl', 0.0)),
            }]

        def get_payment_status(so):
            if so.get('invoice_number'):
                return 'PAID'
            order_status = prestashop_consts.ORDER_STATUS.get(str(so.get('current_state') or 18))
            if order_status in prestashop_consts.PAYMENT_STATE['not_paid']:
                return 'NOT_PAID'
            if order_status in prestashop_consts.PAYMENT_STATE['in_payment']:
                return 'IN_PAYMENT'
            if order_status in prestashop_consts.PAYMENT_STATE['paid']:
                return 'PAID'
            if order_status in prestashop_consts.PAYMENT_STATE['partial']:
                return 'PARTIAL'
            if order_status in prestashop_consts.PAYMENT_STATE['reversed']:
                return 'REVERSED'
            if order_status in prestashop_consts.PAYMENT_STATE['blocked']:
                return 'BLOCKED'
            if order_status in prestashop_consts.PAYMENT_STATE['invoicing_legacy']:
                return 'INVOICING_LEGACY'
            return order_status
        ...
        order_payload = {
            ...
            'financial_status': get_payment_status(so),
            ...
            'order_lines': [{
                ...
                'price_unit': line.get('unit_price_tax_incl', 0) if tax_included else line.get('unit_price_tax_excl', 0),
