...
class PrestashopAPI:
    ...
    def _get_base_url_without_database(self, url):
        ...
        try:
            ...
            if path:
                ...
                return f"{parsed_url.scheme}://{parsed_url.netloc}{new_path}/api"
    ...
    def __get_auth_header(self):
        ...
        if not self.webservice_key:
            raise ECommerceApiError(_("Missing PrestaShop webservice key"))
    ...
    def __api_request(self, method, url, headers, data=False, endpoint_include=False):
        ...
        try:
            return requests.request(method, call_url, headers=headers, data=data, timeout=30)
        except requests.exceptions.Timeout:
            _logger.error('PrestaShop API request timed out: %s', call_url)
            raise ECommerceApiError(_("Timeout while calling PrestaShop API"), error_code=408)
    ...
    def __check_isvalid(self, response):
        ...
        if 'application/json' in content_type:
            try:
                ...
                raise ECommerceApiError(_("Invalid JSON response from PrestaShop"), error_code=status)
    ...
    def __resource_request(
        ...
        if default_filter == 1:
            ...
            url += f'?display={display}{date_filter}{custom_filter}{store_filter}{output_format}'
