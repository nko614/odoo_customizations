...
def _call_shopify_graphql_admin_api(headers, url, query):
    ...
    requests.exceptions.Timeout, requests.exceptions.HTTPError, requests.exceptions.RequestException, requests.exceptions.InvalidURL) as ex:
        _logger.error("Error occuer during execute request on %s, error_description: %s", url, ex)
...
def _handle_query_error(response, url):
    ...
    if response.get('errors'):
        ...
        _logger.error("Error occuer after fetch data from %s, error_description: %s", url, error_message)
...
def _handle_mutation_error(mutation_key, response, url):
    ...
    if userErrors:
        ...
        _logger.error("Error occuer after mutation on %s, error_description: %s", url, error_message)
        raise ECommerceApiError(error_message)
...
def _shopify_prepare_order_structure(order_node):
    ...
    order = {
        'billing_address': {
            ...
            'email': email,
            'name': (billing_address.get('firstName') or '') + (billing_address.get('lastName') or ''),
        ...
        'shipping_address': {
            ...
            'email': email,
            'name': (shipping_address.get('firstName') or '') + (shipping_address.get('lastName') or ''),
...
def _shopify_prepare_order_lines_structure(line_nodes):
    ...
    for line_node in line_nodes:
        order_line = {
            ...
            'discount_tax': _calculate_shopify_tax(line_node.get('taxLines'), _calculate_shopify_discount(line_node.get('discountAllocations'))),
            ...
            'tax_amount': _calculate_shopify_tax(line_node.get('taxLines'), line_node.get('originalUnitPriceSet').get('shopMoney').get('amount'), line_node.get('quantity', 0)),
        }
...
def _shopify_prepare_shipping_lines_structure(line_nodes):
    ...
    for line_node in line_nodes:
        shipping_line = {
            ...
            'tax_amount': _calculate_shopify_tax(line_node.get('taxLines'), line_node.get('originalPriceSet').get('shopMoney').get('amount')),
        }
...
# === Helper methods === #
def _calculate_shopify_tax(tax_lines, price, quantity=1):
    total_tax = 0
    for line in tax_lines:
        total_tax += (float(line.get('rate')) * float(price) * quantity)
    return total_tax
